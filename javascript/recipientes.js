function calcularepFcn() {
    ...
    Vtapas = Math.PI/12*(Math.pow(diametro + 2*espesortapareal,3) - Math.pow(diametro,3));
        ...
        volumenL=Math.pow(R,2)*longitud*(theta-Math.sin(theta)*Math.cos(theta));
        document.querySelector('#volumenL').value = volumenL.toFixed(2);

        Pliquido=volumenL*rho;
        document.querySelector('#Pliquido').value = Pliquido.toFixed(2);

        PequipoLleno=Pliquido+pesoequipo;
        document.querySelector('#PequipoLleno').value = PequipoLleno.toFixed(0);
